...
PID_Controller::PID_Controller(const PID_Params& params, const float outMax, const float maxRate,
                               const float deadband)
    : target(0.0f), params_(params), outMax_(outMax), maxRate_(maxRate), deadband_(deadband),
      prevErr_(0.0f), integral_(0.0f), output_(0.0f) {
}
...
void PID_Controller::update(const float error, const float diff) {
    if (0.0f == this->target && abs(error) < this->deadband_) {
        ...
        const float output =
            error * this->params_.P + this->integral_ * this->params_.I + diff * this->params_.D;

        this->output_ =
            clamp(output, this->output_ - this->maxRate_, this->output_ + this->maxRate_);
