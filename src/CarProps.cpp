...
radian_t CarProps::getSpeedAngle(const meter_t frontRearPivotDist) const {
    const vec2m centerToFrontPivotCenter = vec2m{ frontRearPivotDist / 2, meter_t(0) }.rotate(this->pose.angle);
    ...
    if (eq(this->frontWheelAngle, this->rearWheelAngle)) {
        ...
        const line2m frontWheelPerpendicularLine(frontPivotCenter, this->pose.angle + this->frontWheelAngle + PI_2);
        const line2m rearWheelPerpendicularLine(rearPivotCenter, this->pose.angle + this->rearWheelAngle + PI_2);

        const point2m trajectoryCircleCenter = lineLine_intersection(frontWheelPerpendicularLine, rearWheelPerpendicularLine);
...
vec2mps CarProps::getSpeedVector(const meter_t frontRearPivotDist) const {
    return vec2mps{ abs(this->speed), m_per_sec_t(0) }.rotate(this->getSpeedAngle(frontRearPivotDist));
}
