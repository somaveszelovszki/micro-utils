#include <micro/hw/LSM6DS3_Gyroscope.hpp>
...
constexpr millisecond_t TIMEOUT_INIT(20.0f);
constexpr millisecond_t TIMEOUT(2.0f);
...
Status LSM6DS3_Gyroscope::initialize(void) {
    ...
    micro::nonBlockingDelay(millisecond_t(25));    // time for gyroscope to initialize itself after startup
...
Status LSM6DS3_Gyroscope::read(GyroData * const pResult) {
    Status status;
    uint8_t buffer[6];
