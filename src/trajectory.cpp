...
radian_t Trajectory::lastSpeedAngle() const {
    return this->configs_.size() >= 2
               ? (this->configs_.rend()->pose.pos - std::next(this->configs_.rend())->pose.pos)
                     .getAngle()
               : radian_t(0);
}
...
void Trajectory::appendCircle(const point2m& center, radian_t angle, m_per_sec_t destSpeed) {
    ...
    for (uint32_t i = 1; i <= numSections; ++i) {
        const m_per_sec_t currentSpeed =
            micro::lerp<uint32_t, m_per_sec_t>(i, 0, numSections, lastCfg.speed, destSpeed);
        const vec2m currentRelativeVec = relativeVec.rotate(
            micro::lerp<uint32_t, radian_t>(i, 0, numSections, radian_t(0), angle));
        const radian_t currentFwdAngle = currentRelativeVec.getAngle() + sgn(angle) * PI_2;

        this->appendLine(
            config_t{{center + currentRelativeVec, toCarOrientation(currentFwdAngle, currentSpeed)},
                     currentSpeed});
    }
}

void Trajectory::appendSineArc(const config_t& dest, radian_t fwdAngle,
                               orientationUpdate_t orientationUpdate, radian_t sineStart,
                               radian_t sineEnd) {
    const auto& lastCfg = this->configs_.back();
    ...
    for (uint32_t i = 1; i <= numSections; ++i) {
        ...
        const meter_t y_ =
            c1_.Y +
            dy * (1 - cos(micro::lerp<float, radian_t>(i, 0, numSections, sineStart, sineEnd))) / 2;
        const point2m currentPoint = point2m{x_, y_}.rotate(fwdAngle);
        const m_per_sec_t currentSpeed =
            micro::lerp<uint32_t, m_per_sec_t>(i, 0, numSections, lastCfg.speed, dest.speed);
        const radian_t currentFwdAngle = orientationUpdate == orientationUpdate_t::FIX_ORIENTATION
                                             ? lastCfg.pose.angle
                                             : (currentPoint - prevCfg.pose.pos).getAngle();

        this->appendLine(config_t{{currentPoint, toCarOrientation(currentFwdAngle, currentSpeed)},
                                  currentSpeed});
...
ControlData Trajectory::update(const CarProps& car) {
    const std::pair<configs_t::const_iterator, configs_t::const_iterator> sectionBoundaries =
        getSectionBoundaries(car);

    if (sectionBoundaries.first > this->sectionStartConfig_) {
        for (configs_t::const_iterator it = this->sectionStartConfig_;
             it != sectionBoundaries.first; ++it) {
    ...
    const line2m carPerpendicularLine(
        car.pose.pos, car.pose.pos + vec2m{meter_t(0), meter_t(1)}.rotate(car.pose.angle));
    ...
    const Sign lineSign           = eqWithOverflow360(linePointAngle, car.pose.angle + PI_2, PI_2)
                                        ? Sign::NEGATIVE
                                        : Sign::POSITIVE;

    const meter_t sectionStartDist = sectionBoundaries.first->pose.pos.distance(car.pose.pos);
    const meter_t sectionLength =
        sectionBoundaries.first->pose.pos.distance(sectionBoundaries.second->pose.pos);

    const radian_t fwdAngleDiff =
        normalizePM180(sectionBoundaries.second->pose.angle - sectionBoundaries.first->pose.angle);
    const radian_t targetFwdAngle =
        sectionBoundaries.first->pose.angle +
        micro::lerp(sectionStartDist.get(), 0.0f, sectionLength.get(), radian_t(0), fwdAngleDiff);

    ControlData controlData;
    controlData.speed =
        micro::lerp(sectionStartDist.get(), 0.0f, sectionLength.get(),
                    sectionBoundaries.first->speed, sectionBoundaries.second->speed);
    ...
    controlData.lineControl.actual.angle =
        normalize_into_periodic_interval(sectionLineAngle - car.pose.angle, -PI_2, PI_2);
    controlData.lineControl.target.pos = millimeter_t(0);
    controlData.lineControl.target.angle =
        normalize_into_periodic_interval(sectionLineAngle - targetFwdAngle, -PI_2, PI_2);

    return controlData;
}

bool Trajectory::finished(const CarProps& car, const LineInfo& lineInfo,
                          const meter_t lineDetectedDistanceThreshold) const {
    const meter_t residualDistance = this->length() - this->coveredDistance();
    const LinePattern& pattern =
        (car.speed >= m_per_sec_t(0) ? lineInfo.front : lineInfo.rear).pattern;

    return residualDistance <= meter_t(0) ||
           (residualDistance <= lineDetectedDistanceThreshold && LinePattern::NONE != pattern.type);
}

std::pair<Trajectory::configs_t::const_iterator, Trajectory::configs_t::const_iterator>
Trajectory::getSectionBoundaries(const CarProps& car) const {
    ...
    if (closestConfig == this->configs_.begin()) {
        ...
        if (std::distance(this->sectionStartConfig_, prevConfig) > std::ptrdiff_t{0}) {
            ...
            // then checks if the distance between the closest and the previous configuration has
            // been covered. If yes, then the closest configuration has been reached, and the
            // trajectory should now lead the car towards the next configuration.
        } else if (this->sectionStartConfig_ == prevConfig) {
            otherConfig = car.distance - this->carDistanceAtLastConfig_ >=
                                  closestConfig->pose.pos.distance(prevConfig->pose.pos)
                              ? nextConfig
                              : prevConfig;

            // If the section start is after the previous configuration (probably the closest
            // configuration is the section start), then the car is between the closest and the next
            // configuration.
        } else {
    ...
    return {min(closestConfig, otherConfig), max(closestConfig, otherConfig)};
}
...
Trajectory::configs_t::const_iterator Trajectory::getClosestConfig(const point2m& pos) const {
    ...
    for (configs_t::const_iterator it = this->sectionStartConfig_; it != this->configs_.end();
         ++it) {
