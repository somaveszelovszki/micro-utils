...
HD44780_Lcd::HD44780_Lcd(const wireMode_t wireMode, const displayType_t displayType,
                         const gpio_t& rs, const gpio_t& rw, const gpio_t& en,
                         const std::initializer_list<gpio_t>& data)
    : wireMode_(wireMode), displayType_(displayType), rs_(rs), rw_(rw), en_(en) {
    uint32_t i = 0;
...
Status HD44780_Lcd::initialize() {
    ...
    this->clear();
    this->write(DISPLAY_ON_OFF_CONTROL | OPT_D,
                dataType_t::Command); // LCD on, cursor off, cursor-blink off
...
void HD44780_Lcd::write(const uint8_t data, const dataType_t dataType) {
    gpio_write(this->rs_, dataType_t::Command == dataType
                              ? gpioPinState_t::RESET
                              : gpioPinState_t::SET); // selects command/data register
...
void HD44780_Lcd::writeByte(const uint8_t data) {
    ...
    for (uint8_t i = 0; i < this->numWires(); ++i) {
        gpio_write(this->data_[i],
                   ((data >> i) & 0x01) ? gpioPinState_t::SET : gpioPinState_t::RESET);
    }
