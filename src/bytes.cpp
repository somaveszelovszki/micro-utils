...
int16_t toInt16(const uint8_t bytes[], BitOrder order) {
    int16_t result;
    if (order == BitOrder::ENDIAN_LITTLE) {
        result = static_cast<int32_t>(bytes[0]) | (static_cast<int32_t>(bytes[1]) << 8);
    } else { // ENDIAN_BIG
        result = static_cast<int32_t>(bytes[1]) | (static_cast<int32_t>(bytes[0]) << 8);
    }
    return result;
}
...
int32_t toInt32(const uint8_t bytes[], BitOrder order) {
    ...
    if (order == BitOrder::ENDIAN_LITTLE) {
        ...
        result = static_cast<int32_t>(bytes[3]) | (static_cast<int32_t>(bytes[2]) << 8) |
