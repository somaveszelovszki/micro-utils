...
MPU9250_Gyroscope::MPU9250_Gyroscope(const spi_t& spi, const gpio_t& cs, Ascale aScale,
                                     Gscale gScale, Mscale mScale, uint8_t Mmode)
    : spi(spi), cs(cs), aScale(aScale), gScale(gScale), mScale(mScale), aRes(getAres(aScale)),
      gRes(getGres(gScale)), mRes(getMres(mScale)), Mmode(Mmode), magCalibration{0.0f, 0.0f, 0.0f},
      gyroBias{0.0f, 0.0f, 0.0f}, gyroThreshold{0.0f, 0.0f, 0.0f}, accelBias{0.0f, 0.0f, 0.0f} {
}

void MPU9250_Gyroscope::writeByte(uint8_t address, uint8_t subAddress, uint8_t data) {
    uint8_t txRxData[2] = {subAddress, data};

    gpio_write(this->cs, gpioPinState_t::RESET);
    spi_exchange(this->spi, txRxData, nullptr, 2);
    gpio_write(this->cs, gpioPinState_t::SET);
}
...
bool MPU9250_Gyroscope::readBytes(uint8_t address, uint8_t subAddress, uint8_t count,
                                  uint8_t* dest) {
    bool isOk = false;

    constexpr uint8_t MAX_COUNT = 8;
    uint8_t txRxData[MAX_COUNT] = {0, 0, 0, 0, 0, 0, 0, 0};
    const uint8_t transferSize  = std::min<uint8_t>(count, MAX_COUNT - 1);

    txRxData[0] = subAddress | 0x80;
    gpio_write(this->cs, gpioPinState_t::RESET);
    spi_exchange(this->spi, txRxData, txRxData, transferSize);
    memcpy(dest, &txRxData[1], transferSize);
    gpio_write(this->cs, gpioPinState_t::SET);
...
point3<m_per_sec2_t> MPU9250_Gyroscope::readAccelData() {
    ...
    if (this->readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0])) {
        result =
            static_cast<point3<m_per_sec2_t>>(bufferToRaw(rawData)) * this->aRes * micro::G.get();
    } else {
...
point3<rad_per_sec_t> MPU9250_Gyroscope::readGyroData() {
    ...
    if (this->readRawGyroData(gyroData)) {
        ...
        if (abs(gyroData.X) < this->gyroThreshold.X)
            gyroData.X = 0.0f;
        if (abs(gyroData.Y) < this->gyroThreshold.Y)
            gyroData.Y = 0.0f;
        if (abs(gyroData.Z) < this->gyroThreshold.Z)
            gyroData.Z = 0.0f;
...
point3<gauss_t> MPU9250_Gyroscope::readMagData() {
    ...
    uint8_t rawData[7];
    if (this->readByte(AK8963_ADDRESS, AK8963_ST1) &
        0x01) { // wait for magnetometer data ready bit to be set
        this->readBytes(
            AK8963_ADDRESS, AK8963_XOUT_L, 7,
...
void MPU9250_Gyroscope::reset() {
    this->writeByte(MPU9250_ADDRESS, PWR_MGMT_1,
...
void MPU9250_Gyroscope::initAK8963() {
    ...
    uint8_t rawData[3];
    this->readBytes(AK8963_ADDRESS, AK8963_ASAX, 3,
    ...
    // 100 Hz sample rates
    this->writeByte(AK8963_ADDRESS, AK8963_CNTL,
                    (((uint8_t)this->mScale << 4) |
                     this->Mmode)); // Set magnetometer data resolution and sample ODR
    os_sleep(millisecond_t(10));
}
...
void MPU9250_Gyroscope::initMPU9250() {
    ...
    os_sleep(millisecond_t(100)); // Delay 100 ms for PLL to get established on x-axis gyro; should
                                  // check for PLL ready interrupt

    this->writeByte(
        MPU9250_ADDRESS, USER_CTRL,
        0x10); // sets I2C_IF_DIS flag (disables I2C, puts serial interface in SPI mode only)

    // get stable time source
    this->writeByte(
        MPU9250_ADDRESS, PWR_MGMT_1,
    ...
    // Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions
    // 4:3
    uint8_t c =
        this->readByte(MPU9250_ADDRESS, GYRO_CONFIG); // get current GYRO_CONFIG register value
...
point3<int16_t> MPU9250_Gyroscope::bufferToRaw(const uint8_t rawData[6]) {
    return {static_cast<int16_t>((static_cast<uint16_t>(rawData[0]) << 8) | rawData[1]),
...
void MPU9250_Gyroscope::initialize() {
    ...
    this->initMPU9250();
    this->calibrateGyro();
    LOG_DEBUG("Gyro initialized. Bias:  {}, {}, {}", this->gyroBias.X, this->gyroBias.Y,
              this->gyroBias.Z);
}
