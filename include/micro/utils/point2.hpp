...
template <typename T> struct point2 {
    ...
    point2(const T& fill = T()) : X(fill), Y(fill) {}
     ...
     **/
    template <typename T2> bool operator==(const point2<T2>& other) const {
     ...
     **/
    T length() const { return micro::pythag(this->X, this->Y); }

    /* @brief Calculates distance between the two points.
     * @param other The other point.
     * @returns The distance between the two points.
     **/
    template <typename T2> T distance(point2<T2> other) const {
        return micro::pythag(this->X - other.X, this->Y - other.Y);
    }

    /* @brief Calculates angle of the vector.
     **/
    radian_t getAngle() const { return micro::atan2(this->Y, this->X); }

    /* @brief Calculates angle of given vector using this point as the origo.
     * @param other The vector.
     **/
    template <typename T2> radian_t getAngle(const vec2<T2>& other) const {
    ...
    point2<T> average(const point2<T2>& other, float otherWeight = 1.0f) const {
        ...
        return point2<T>((this->X + other.X * otherWeight) / weightSum,
                         (this->Y + other.Y * otherWeight) / weightSum);
    }

    point2<T> rotate(const float c, const float s) const {
        return {c * this->X - s * this->Y, s * this->X + c * this->Y};
    }

    point2<T> rotate(const radian_t& angle) const { return this->rotate(cos(angle), sin(angle)); }

    point2<T> rotate90() const { return this->rotate(0.0f, 1.0f); }

    point2<T> rotate180() const { return this->rotate(-1.0f, 0.0f); }

    point2<T> rotate270() const { return this->rotate(0.0f, -1.0f); }
 ...
 * @returns This point.
 **/
template <typename T1, typename T2> point2<T1>& operator+=(point2<T1>& p1, const point2<T2>& p2) {
    p1.X += p2.X;
    p1.Y += p2.Y;
    return p1;
}

/* @brief Subtracts coordinates of the other point from the coordinates of this point and stores the
...
typename std::enable_if<!is_base_of_template<point2, T2>::value, point2<R>>::type
operator*(const point2<T1>& p, const T2& c) {
...
template <typename T1, typename T2, typename R = decltype(std::declval<T1>() * std::declval<T2>())>
typename std::enable_if<!is_base_of_template<point2, T2>::value, point2<R>>::type
operator*(const T1& c, const point2<T2>& p) {
    return p * c;
}
...
/* @brief Multiplies coordinates of the point with the given constant and stores the result in the
 * point.
 * @param c The constant.
 * @returns This point.
 **/
template <typename T1, typename T2>
typename std::enable_if<std::is_arithmetic<T2>::value, point2<T1>&>::type operator*=(point2<T1>& p,
                                                                                     const T2& c) {
    p.X *= c;
    p.Y *= c;
    return p;
 ...
 **/
template <typename T1, typename T2>
typename std::enable_if<std::is_arithmetic<T2>::value, point2<T1>&>::type
operator*=(const T2& c, const point2<T1>& p) {
    return p * c;
}
...
template <typename T> bool point2<T>::isInside(const bbox2<T>& bbox) const {
    return this->X >= bbox.bl.X && this->X < bbox.tr.X && this->Y >= bbox.bl.Y &&
           this->Y < bbox.tr.Y;
}
