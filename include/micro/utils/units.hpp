...
template <typename T, typename partial = void> struct is_unit : std::false_type {};

template <typename T> struct is_unit<T, std::enable_if_t<T::is_dim_class>> : std::true_type {};

template <typename T> inline constexpr bool is_unit_v = is_unit<T>::value;

template <typename T1, typename T2, typename partial = void>
struct is_same_unit_dimension : std::false_type {};

template <typename T1, typename T2>
struct is_same_unit_dimension<
    T1, T2, std::enable_if_t<is_unit_v<T1> && is_unit_v<T2> && T1::dim == T2::dim>>
    : std::true_type {};
...
constexpr float DEG_TO_RAD = 0.0174532925f; // Converts from degrees to radians.

/* @brief Multiplication dimension mapper. Maps result dimension of the two given dimension's
 * multiplication (e.g. capacity for current and time).
...
/* @brief Division dimension mapper. Maps result dimension of the two given dimension's division
 * (e.g. speed for distance and time).
 * @tparam _first_dim The first dimension.
 * @tparam _second_dim The second dimension.
 **/
...
#define dimension_connections(dim1, dim2, dim3)                                                    \
    namespace detail {                                                                             \
    template <> struct mul_dim<dim1, dim2> {                                                       \
        static constexpr Dimension value = dim3;                                                   \
    };                                                                                             \
    template <> struct mul_dim<dim2, dim1> {                                                       \
        static constexpr Dimension value = dim3;                                                   \
    };                                                                                             \
    template <> struct div_dim<dim3, dim1> {                                                       \
        static constexpr Dimension value = dim2;                                                   \
    };                                                                                             \
    template <> struct div_dim<dim3, dim2> {                                                       \
        static constexpr Dimension value = dim1;                                                   \
    };                                                                                             \
    } // namespace detail

#define square_dimension_connections(dim, sq_dim)                                                  \
    namespace detail {                                                                             \
    template <> struct mul_dim<dim, dim> {                                                         \
        static constexpr Dimension value = sq_dim;                                                 \
    };                                                                                             \
    template <> struct div_dim<sq_dim, dim> {                                                      \
        static constexpr Dimension value = dim;                                                    \
    };                                                                                             \
    } // namespace detail
...
template <> struct unit_multiplier<Unit::giga> {
    static constexpr float value = 1000000000.0f;
}; // Unit multiplier for giga.
template <> struct unit_multiplier<Unit::mega> {
    static constexpr float value = 1000000.0f;
}; // Unit multiplier for mega.
template <> struct unit_multiplier<Unit::kilo> {
    static constexpr float value = 1000.0f;
}; // Unit multiplier for kilo.
template <> struct unit_multiplier<Unit::hecto> {
    static constexpr float value = 100.0f;
}; // Unit multiplier for hecto.
template <> struct unit_multiplier<Unit::deca> {
    static constexpr float value = 10.0f;
}; // Unit multiplier for deca.
template <> struct unit_multiplier<Unit::one> {
    static constexpr float value = 1.0f;
}; // Unit multiplier for one.
template <> struct unit_multiplier<Unit::deci> {
    static constexpr float value = 1 / 10.0f;
}; // Unit multiplier for deci.
template <> struct unit_multiplier<Unit::centi> {
    static constexpr float value = 1 / 100.0f;
}; // Unit multiplier for centi.
template <> struct unit_multiplier<Unit::milli> {
    static constexpr float value = 1 / 1000.0f;
}; // Unit multiplier for milli.
template <> struct unit_multiplier<Unit::micro> {
    static constexpr float value = 1 / 1000000.0f;
}; // Unit multiplier for micro.
template <> struct unit_multiplier<Unit::nano> {
    static constexpr float value = 1 / 1000000000.0f;
}; // Unit multiplier for nano.

// custom unit multipliers
template <> struct unit_multiplier<Unit::_60> {
    static constexpr float value = 60.0f;
}; // Unit multiplier for _60.
template <> struct unit_multiplier<Unit::_3600> {
    static constexpr float value = 3600.0f;
}; // Unit multiplier for _3600.
template <> struct unit_multiplier<Unit::deg_to_rad> {
    static constexpr float value = DEG_TO_RAD;
}; // Unit multiplier for rad_to_deg.

template <Dimension _dim> struct base_unit_instance {
    static constexpr Dimension dim = _dim; // The dimension.
};

template <typename T> struct is_unit_instance {
 ...
 **/
template <Dimension _dim, Unit _unit> struct unit_instance : public base_unit_instance<_dim> {
    static constexpr float mul = unit_multiplier<_unit>::value; // The unit multiplier.
};
 ...
 **/
template <typename T1, typename T2,
          class = typename std::enable_if<is_unit_instance<T1>::value &&
                                          is_unit_instance<T2>::value>::type>
struct mul_unit_instance : public base_unit_instance<mul_dim<T1::dim, T2::dim>::value> {
    static constexpr float mul = T1::mul * T2::mul; // The unit multiplier.
};
 ...
 **/
template <typename from_unit_inst_t, typename to_unit_inst_t, typename T,
          class = typename std::enable_if<std::is_arithmetic<T>::value>::type>
inline constexpr typename std::enable_if<from_unit_inst_t::dim == to_unit_inst_t::dim, T>::type
rescale_unit(const T& value) {
 ...
 **/
template <Dimension _dim, typename unit_inst_t_ = unit_instance<_dim, Unit::one>,
...
class dim_class {
  public:
    ...
    static_assert(_dim == unit_inst_t::dim,
                  "Dimensions do not match!"); // Checks if the dimensions match.

  private:
    template <Dimension dim2, typename unit_inst_t2, bool explicit_unit2> friend class dim_class;
    template <typename unit_inst_t2, bool explicit_unit2>
    using same_dim_class = dim_class<dim, unit_inst_t2, explicit_unit2>;
    template <Dimension dim2, typename unit_inst_t2, bool explicit_unit2>
    using other_dim_class = dim_class<dim2, unit_inst_t2, explicit_unit2>;
  ...
  public:
     ...
     * @restrict Dimension of the result instance type must be the same as the dimension of this
     * type.
 ...
 **/
template <typename T1, typename T2>
inline constexpr typename std::enable_if<is_same_unit_dimension<T1, T2>::value, T1>::type
operator+(const T1& d1, const T2& d2) {
 ...
 **/
template <typename T1, typename T2>
inline typename std::enable_if<is_same_unit_dimension<T1, T2>::value, void>::type
operator+=(T1& d1, const T2& d2) {
 ...
 **/
template <typename T1, typename T2>
inline typename std::enable_if<is_same_unit_dimension<T1, T2>::value, bool>::type
operator==(const T1& d1, const T2& d2) {
...
/* @brief Compares two dimension class instances.
 * @param other The other dimension class instance.
 * @returns Boolean value indicating if this dimension class instances is greater-or-equal than the
 * other.
 **/
template <typename T1, typename T2>
inline typename std::enable_if<is_same_unit_dimension<T1, T2>::value, bool>::type
...
/* @brief Compares two dimension class instances.
 * @param other The other dimension class instance.
 * @returns Boolean value indicating if this dimension class instances is smaller-or-equal than the
 * other.
 **/
template <typename T1, typename T2>
inline typename std::enable_if<is_same_unit_dimension<T1, T2>::value, bool>::type
 ...
 **/
template <typename T1, typename T2>
inline constexpr typename std::enable_if<is_same_unit_dimension<T1, T2>::value, float>::type
operator/(const T1& d1, const T2& d2) {
    return d1.template get<true>() / static_cast<T1>(d2).template get<true>();
}
...
template <
    ...
    typename R = dim_class<mul_dim<T1::dim, T2::dim>::value,
                           mul_unit_instance<typename T1::unit_inst_t, typename T2::unit_inst_t>>>
inline constexpr typename std::enable_if<is_unit<T1>::value && is_unit<T2>::value, R>::type
operator*(const T1& d1, const T2& d2) {
...
template <
    typename T1, typename T2,
    typename R = dim_class<div_dim<T1::dim, T2::dim>::value,
                           div_unit_instance<typename T1::unit_inst_t, typename T2::unit_inst_t>>>
inline constexpr
    typename std::enable_if<is_unit<T1>::value && is_unit<T2>::value && T1::dim != T2::dim, R>::type
    operator/(const T1& d1, const T2& d2) {
    return R(d1.template get<true>() / d2.template get<true>(), nullptr);
}
...
template <typename T1, typename T2>
inline constexpr
    typename std::enable_if<is_unit<T1>::value && std::is_arithmetic<T2>::value, T1>::type
    operator*(const T1& d, const T2& c) {
...
inline constexpr
    typename std::enable_if<is_unit<T1>::value && std::is_arithmetic<T2>::value, T1>::type
    operator*(const T2& c, const T1& d) {
    return d * c;
}
 ...
 **/
template <typename T1, typename T2>
inline typename std::enable_if<is_unit<T1>::value && std::is_arithmetic<T2>::value, T1&>::type
operator*=(T1& d, const T2& c) {
...
dimension_connections(Dimension::speed, Dimension::time,
                      Dimension::distance) // Dimension connections for speed, time and distance
                                           // (speed * time = distance).
    dimension_connections(Dimension::acceleration, Dimension::time,
                          Dimension::speed) // Dimension connections for acceleration, time and
                                            // speed (acceleration * time = speed).
    dimension_connections(Dimension::angular_velocity, Dimension::time,
                          Dimension::angle) // Dimension connections for angular velocity, time and
                                            // angle (angular velocity * time = angle).

    namespace detail {
    template <Dimension dim> struct mul_dim<Dimension::angle, dim> {
        static constexpr Dimension value = dim;
    };
    template <Dimension dim> struct mul_dim<dim, Dimension::angle> {
        static constexpr Dimension value = dim;
    };
    template <Dimension dim> struct div_dim<dim, Dimension::angle> {
        static constexpr Dimension value = dim;
    };
} // namespace detail

#define create_unit_instance_with_unit_prefix(dim, mul, unit)                                      \
    typedef detail::dim_class<Dimension::dim, detail::unit_instance<Dimension::dim, Unit::mul>,    \
                              true>                                                                \
        mul##unit##_t;                                                                             \
    typedef detail::dim_class<                                                                     \
        Dimension::dim,                                                                            \
        detail::square_unit_instance<detail::unit_instance<Dimension::dim, Unit::mul>>, true>      \
        mul##unit##2_t;
...
#define create_mul_unit_instance(unit1, unit2, unit)                                               \
    typedef detail::dim_class<                                                                     \
        detail::mul_unit_instance<unit1##_t::unit_inst_t, unit2##_t::unit_inst_t>::dim,            \
        detail::mul_unit_instance<unit1##_t::unit_inst_t, unit2##_t::unit_inst_t>, true>           \
        unit##_t;

#define create_div_unit_instance(unit1, unit2, unit)                                               \
    typedef detail::dim_class<                                                                     \
        detail::div_unit_instance<unit1##_t::unit_inst_t, unit2##_t::unit_inst_t>::dim,            \
        detail::div_unit_instance<unit1##_t::unit_inst_t, unit2##_t::unit_inst_t>, true>           \
        unit##_t;
