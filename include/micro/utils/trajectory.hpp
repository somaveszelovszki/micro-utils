...
class Trajectory {
  public:
    enum class orientationUpdate_t { FIX_ORIENTATION = 1, PATH_ORIENTATION = 2 };
    ...
    Trajectory()
        : sectionStartConfig_(this->configs_.end()), length_(0), coveredDistanceUntilLastConfig_(0),
          carDistanceAtLastConfig_(0), carDistanceSinceLastConfig_(0) {}

    meter_t length() const { return this->length_; }

    meter_t coveredDistance() const {
        return this->coveredDistanceUntilLastConfig_ + carDistanceSinceLastConfig_;
    }

    config_t lastConfig() const { return this->configs_.back(); }
    ...
    void appendSineArc(const config_t& dest, radian_t fwdAngle,
                       orientationUpdate_t orientationUpdate, radian_t sineStart, radian_t sineEnd);

    ControlData update(const CarProps& car);

    bool finished(const CarProps& car, const LineInfo& lineInfo,
  ...
  private:
    std::pair<configs_t::const_iterator, configs_t::const_iterator>
    getSectionBoundaries(const CarProps& car) const;
