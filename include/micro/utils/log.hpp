...
#define LOG_DEBUG(format, ...) micro::debug::printlog(LogLevel_Debug, format, ##__VA_ARGS__)
#else
#define LOG_DEBUG(...)
#endif

#if MIN_LOG_LEVEL <= LogLevel_Info
#define LOG_INFO(format, ...) micro::debug::printlog(LogLevel_Info, format, ##__VA_ARGS__)
#else
#define LOG_INFO(...)
#endif

#if MIN_LOG_LEVEL <= LogLevel_Warning
#define LOG_WARNING(format, ...) micro::debug::printlog(LogLevel_Warning, format, ##__VA_ARGS__)
#else
#define LOG_WARNING(...)
#endif

#if MIN_LOG_LEVEL <= LogLevel_Error
#define LOG_ERROR(format, ...) micro::debug::printlog(LogLevel_Error, format, ##__VA_ARGS__)
#else
#define LOG_ERROR(...)
#endif
