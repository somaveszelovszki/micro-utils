...
struct LinePattern {
  public:
    ...
    enum type_t : uint8_t {
        ...
        LANE_CHANGE = 4, ///< 1 solid line in the middle, 1 dashed line on one (UNKNOWN!) side (dash
                         ///< and space lengths decreasing)
    ...
    Sign dir    = Sign::NEUTRAL; // The pattern direction (POSITIVE means it is the same as the car
                                 // direction).
    Direction side = Direction::CENTER; // The side of the line the pattern is on. In case of
                                        // symmetrical pattern: CENTER.
    meter_t startDist;
    ...
    bool operator!=(const LinePattern& other) const { return !(*this == other); }
};
...
enum class linePatternDomain_t : uint8_t { Race = 0, Labyrinth = 1 };
