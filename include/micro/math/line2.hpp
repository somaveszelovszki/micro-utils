...
template <typename T> struct line2 {
    ...
    line2(const T& a, const T& b, const T& c) : a(a), b(b), c(c) { this->normalize(); }

    line2(const point2<T>& p1, const point2<T>& p2) { this->fromPoints(p1, p2); }

    line2(const point2<T>& p, radian_t angle) { this->fromPointAndAngle(p, angle); }

    /* @brief Casts line to another type.
     * @returns Line cast to another type.
     **/
    template <typename T2> operator line2<T2>() const { return line2<T2>(T2(a), T2(b), T2(c)); }
    ...
    T getY(const T& x) const { return isZero(b) ? -(c / a) : -(a / b) * x - (c / b); }

    T getX(const T& y) const { return isZero(a) ? -(c / b) : -(b / a) * y - (c / a); }

    void fromPoints(const point2<T>& p1, const point2<T>& p2) {
        a = p1.Y - p2.Y;
        b = p2.X - p1.X;
        c = T(underlying_value(p1.X) * underlying_value(p2.Y) -
              underlying_value(p2.X) * underlying_value(p1.Y));
